def main(ruta, ruta_salida):
    ...
    def convertir_a_pseudocodigo(lineas_codigo):
        ...
        for linea in lineas_codigo:
            ...
            elif linea.startswith('print(') or linea.startswith('print(f'):
                imprimir = linea.split('print(')[1].rstrip(')').replace("{", "").replace("}", "").replace("\\", "")
                if (imprimir.startswith('f')):
                    imprimir = imprimir[1:]
                if (imprimir.startswith("'n")):
                    imprimir =imprimir[:1] + imprimir[2:]

                imprimir=imprimir.replace("'", '"')
                pseudocodigo.append(f"ESCRIBIR {imprimir}")
